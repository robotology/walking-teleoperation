...
bool RobotControlInterface::configure(const yarp::os::Searchable &config,
  ...
  if (!YarpHelper::getStringFromSearchable(config, "remote_sensor_boards",
                                           iCubSensorPart)) {
    yError() << "[RobotControlInterface::configure] Unable to find "
  ...
  optionsAnalogDevice.put("local",
                          "/" + robot + "/" + iCubSensorPart + "/analog:i");
  optionsAnalogDevice.put("remote",
                          "/" + robot + "/" + iCubSensorPart + "/analog:o");
  ...
  if (!m_analogDevice.view(m_AnalogSensorInterface) ||
      !m_AnalogSensorInterface) {
  ...
  m_noAnalogSensor =
      config.check("noAnalogSensor", yarp::os::Value(15)).asInt();
  ...
  if (!YarpHelper::yarpListToStringVector(jointFeedbackTypeListYarp,
                                          jointFeedbackTypeList)) {
    yError() << "[RobotControlInterface::configure] Unable to convert yarp "
  ...
  for (std::vector<std::string>::iterator it = jointFeedbackTypeList.begin();
       it != jointFeedbackTypeList.end(); ++it) {
  ...
  for (size_t i = 0; i < m_axesList.size(); i++) {
    ...
    if (!config.check(axisInfoObj.axisName, axisJointListYarp)) {
      yError() << "[RobotControlInterface::configure] Unable to find "
               << axisInfoObj.axisName << "into config file.";
      return false;
    }
    ...
    for (std::vector<std::string>::iterator it = axisJointList.begin();
         it != axisJointList.end(); ++it) {
      m_actuatedJointList.push_back(*it);

      auto elementJoint =
          std::find(std::begin(jointList), std::end(jointList), *it);
      ...
      if (axisInfoObj.useAnalog) {
        auto elementAnalog =
            std::find(std::begin(analogList), std::end(analogList), *it);
        if (elementAnalog == std::end(analogList)) {
          yError() << "[RobotControlInterface::configure] the joint name ("
                   << *it
  ...
  if (!YarpHelper::getYarpVectorFromSearchable(config, "joints_min_boundary",
                                               m_joints_min_boundary)) {
    yError() << "RobotControlInterface::configure] unable to get the minimum "
                "boundary of the joints limits.";
    return false;
  }

  if (!YarpHelper::getYarpVectorFromSearchable(config, "joints_max_boundary",
                                               m_joints_max_boundary)) {
  ...
  if (!YarpHelper::getYarpVectorFromSearchable(config, "sensors_min_boundary",
                                               m_sensors_min_boundary)) {
    yError() << "RobotControlInterface::configure] unable to get the minimum "
                "boundary of the "
                "joints limits.";
    return false;
  ...
  // get the sensors limits boundaries
  if (!YarpHelper::getYarpVectorFromSearchable(config, "sensors_max_boundary",
                                               m_sensors_max_boundary)) {
    yError() << "RobotControlInterface::configure] unable to get the maximum "
                "boundary of the "
                "joints limits.";
    return false;
  ...
  YarpHelper::addVectorOfStringToProperty(optionsRobotDevice, "axesNames",
                                          m_axesList);
  ...
  bool useVelocity =
      config.check("useVelocity", yarp::os::Value(false)).asBool();
  m_controlMode = useVelocity ? VOCAB_CM_VELOCITY : VOCAB_CM_POSITION_DIRECT;
  m_pidControlMode = useVelocity ? yarp::dev::VOCAB_PIDTYPE_VELOCITY
  ...
  if (!m_robotDevice.view(m_positionDirectInterface) ||
      !m_positionDirectInterface) {
    yError() << "[RobotControlInterface::configure] Cannot obtain "
  ...
  if (!m_robotDevice.view(m_timedInterface) || !m_timedInterface) {
    yError()
        << "[RobotControlInterface::configure] Cannot obtain iTimed interface";
    return false;
  }
  ...
  for (int i = 0; i < 10 && !okPosition; i++) {
    okPosition = m_encodersInterface->getEncoders(
        m_encoderPositionFeedbackInDegrees.data());
...
bool RobotControlInterface::setDirectPositionReferences(
    const yarp::sig::Vector &desiredPosition) {
...
bool RobotControlInterface::setPositionReferences(
    const yarp::sig::Vector &desiredPosition) {
  ...
  // set desired position
  if (!m_positionInterface->positionMove(m_desiredJointValue.data()) &&
      m_isMandatory) {
...
bool RobotControlInterface::setVelocityReferences(
...
bool RobotControlInterface::setCurrentReferences(
    const yarp::sig::Vector &desiredCurrent) {
  ...
  // set desired current
  if (!m_currentInterface->setRefCurrents(m_desiredCurrent.data()) &&
      m_isMandatory) {
...
bool RobotControlInterface::setPwmReferences(
    const yarp::sig::Vector &desiredPwm) {
  ...
  if (!m_pwmInterface->setRefDutyCycles(m_pwmDesired.data()) && m_isMandatory) {
    yError()
        << "[RobotControlInterface::setPwmReferences] Error while setting the "
           "desired PWM.";
    return false;
  }
  return true;
...
bool RobotControlInterface::getFeedback() {
  ...
  if (!m_encodersInterface->getEncoders(
          m_encoderPositionFeedbackInDegrees.data()) &&
      m_isMandatory) {
    yError()
        << "[RobotControlInterface::getFeedbacks] Unable to get joint position";
    return false;
  }
  ...
  if (!m_encodersInterface->getEncoderSpeeds(
          m_encoderVelocityFeedbackInDegrees.data()) &&
      m_isMandatory) {
  ...
  if (!m_currentInterface->getCurrents(m_currentFeedback.data()) &&
      m_isMandatory) {
  ...
  if (!m_pidInterface->getPidOutputs(m_pidControlMode, m_pidOutput.data()) &&
      m_isMandatory) {
    yError()
        << "[RobotControlInterface::getFeedbacks] Unable to get pid outputs";
    return false;
  }
...
bool RobotControlInterface::getCalibratedFeedback() {
  ...
  for (unsigned j = 0; j < m_noAnalogSensor; ++j) {
    ...
    m_analogSensorFeedbackInDegrees(j) =
        m_joints_min_boundary(j) + m_sensors_raw2Degree_scaling(j) *
                                       (m_analogSensorFeedbackRaw(j) -
                                        m_sensors_min_boundary(j)); // TOCHECK

    m_analogSensorFeedbackInRadians(j) =
        iDynTree::deg2rad(m_analogSensorFeedbackInDegrees(j));
  }
  ...
  for (size_t i = 0; i < m_axisInfoList.size(); ++i) {
    if (m_axisInfoList[i].useAnalog) {
      ...
      for (std::vector<int>::iterator it =
           ...
           it != m_axisInfoList[i].relatedAnalogSensorsIndex.end(); ++it) {
        m_SensorActuatedJointFeedbackInRadians(idx) =
            m_analogSensorFeedbackInRadians(*it);
        idx++;
      }
    } else {
      m_SensorActuatedJointFeedbackInRadians(idx) =
          m_encoderPositionFeedbackInRadians(i);
      idx++;
    }
  }
...
yarp::os::Stamp &RobotControlInterface::timeStamp() { return m_timeStamp; }
...
bool RobotControlInterface::getLimits(yarp::sig::Matrix &limits) {
  ...
  for (int i = 0; i < m_noActuatedAxis; i++) {
    // get position limits
    if (!m_limitsInterface->getLimits(i, &minLimitInDegree,
                                      &maxLimitInDegree)) {
      if (m_isMandatory) {
        yError() << "[RobotControlInterface::getLimits] Unable get "
                 << m_axesList[i] << " joint limits.";
        return false;
      } else {
...
bool RobotControlInterface::getVelocityLimits(yarp::sig::Matrix &limits) {
  ...
  for (int i = 0; i < m_noActuatedAxis; i++) {
    // get position limits
    if (!m_limitsInterface->getVelLimits(i, &minLimitInDegree,
                                         &maxLimitInDegree)) {
...
bool RobotControlInterface::setJointReference(
  ...
  default:
    yError()
        << "[RobotControlInterface::setJointReference] Unknown control mode.";
    return false;
  }
