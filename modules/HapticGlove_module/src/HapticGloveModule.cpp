...
bool HapticGloveModule::configure(yarp::os::ResourceFinder &rf) {
  ...
  if (!YarpHelper::getStringFromSearchable(rf, "name", name)) {
    yError()
        << "[OculusModule::configure] Unable to get a string from a searchable";
    return false;
  }
  setName(name.c_str());
  ...
  m_moveRobot =
      generalOptions.check("enableMoveRobot", yarp::os::Value(1)).asBool();
  yInfo() << "[HapticGloveModule::configure] move the robot: " << m_moveRobot;

  // check if log the data
  m_enableLogger =
      generalOptions.check("enableLogger", yarp::os::Value(0)).asBool();
  yInfo() << "[HapticGloveModule::configure] enable the logger: "
          << m_enableLogger;

  // check time period
  m_calibrationTimePeriod =
      generalOptions.check("calibrationTimePeriod", yarp::os::Value(10.0))
          .asDouble();
  yInfo() << "[HapticGloveModule::configure] calibration time period: "
          << m_calibrationTimePeriod;

  double smoothingTime =
      generalOptions.check("smoothingTime", yarp::os::Value(1.0)).asDouble();
  yInfo() << "[HapticGloveModule::configure] smoothingTime :" << smoothingTime;

  // robot name:
  m_robot =
      generalOptions.check("robot", yarp::os::Value("icubSim")).asString();
  ...
  m_useLeftHand =
      generalOptions.check("useLeftHand", yarp::os::Value(1)).asBool();
  m_useRightHand =
      generalOptions.check("useRightHand", yarp::os::Value(1)).asBool();
  yInfo() << "[HapticGloveModule::configure] use the left hand: "
          << m_useLeftHand;
  yInfo() << "[HapticGloveModule::configure] use the right hand: "
          << m_useRightHand;
  ...
  if (m_useLeftHand) {
    ...
    yarp::os::Bottle &leftFingersOptions =
        rf.findGroup("LEFT_FINGERS_RETARGETING");
    ...
    */
    ...
    size_t noRobotAllAxis =
        m_robotLeftHand->controlHelper()->getNumberOfAllAxis();
    size_t noRobotActuatedAxis =
        m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis();
    std::vector<std::string> robotActuatedJointNameList;
    m_robotLeftHand->controlHelper()->getActuatedJointNameList(
        robotActuatedJointNameList);
    std::vector<std::string> robotActuatedAxisNameList;
    m_robotLeftHand->controlHelper()->getActuatedAxisNameList(
    ...
    m_retargetingLeftHand = std::make_unique<Retargeting>(
        noRobotAllAxis, noRobotActuatedAxis, noBuzzMotors,
        robotActuatedJointNameList, robotActuatedAxisNameList,
        humanJointNameList);
  ...
  if (m_useRightHand) {
    ...
    yarp::os::Bottle &rightFingersOptions =
        rf.findGroup("RIGHT_FINGERS_RETARGETING");
    ...
    */
    ...
    size_t noRobotAllAxis =
        m_robotRightHand->controlHelper()->getNumberOfAllAxis();
    size_t noRobotActuatedAxis =
        m_robotRightHand->controlHelper()->getNumberOfActuatedAxis();
    std::vector<std::string> robotActuatedJointNameList;
    m_robotRightHand->controlHelper()->getActuatedJointNameList(
        robotActuatedJointNameList);
    std::vector<std::string> robotActuatedAxisNameList;
    m_robotRightHand->controlHelper()->getActuatedAxisNameList(
        robotActuatedAxisNameList);
    ...
    m_retargetingRightHand = std::make_unique<Retargeting>(
        noRobotAllAxis, noRobotActuatedAxis, noBuzzMotors,
        robotActuatedJointNameList, robotActuatedAxisNameList,
        humanJointNameList);
  ...
  if (m_useLeftHand) {
    ...
    yarp::sig::Vector buff(
        m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis(), 0.0);
    m_leftAxisValueErrorSmoother = std::make_unique<iCub::ctrl::minJerkTrajGen>(
        m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis(), m_dT,
        smoothingTime);
    m_leftAxisValueErrorSmoother->init(buff);

    m_leftAxisVelocityErrorSmoother =
        std::make_unique<iCub::ctrl::minJerkTrajGen>(
            m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis(), m_dT,
            smoothingTime);
    m_leftAxisVelocityErrorSmoother->init(buff);

    // HUMAN
    m_gloveLeftBuzzMotorReference.resize(m_gloveLeftHand->getNoOfBuzzMotors(),
                                         0.0);
    m_gloveLeftForceFeedbackReference.resize(
        m_gloveLeftHand->getNoOfForceFeedback(), 0.0);
  }
  std::cerr << "conf 06 \n";
  if (m_useRightHand) {
    ...
    m_rightAxisValueErrorSmoother =
        std::make_unique<iCub::ctrl::minJerkTrajGen>(
            m_robotRightHand->controlHelper()->getNumberOfActuatedAxis(), m_dT,
            smoothingTime);
    ...
    // HUMAN
    m_gloveRightBuzzMotorReference.resize(m_gloveRightHand->getNoOfBuzzMotors(),
                                          0.0);
    m_gloveRightForceFeedbackReference.resize(
        m_gloveRightHand->getNoOfForceFeedback(), 0.0);
  }
...
bool calibrateRobotMotorsJointsCoupling() { return true; }
double HapticGloveModule::getPeriod() { return m_dT; }
...
bool HapticGloveModule::evaluateRobotFingersReferences() { return true; }
...
bool HapticGloveModule::getFeedbacks() {
  ...
  if (m_useLeftHand) {
    ...
    m_robotLeftHand->getFingerAxisValueReference(
        m_icubLeftFingerAxisValueReference);

    m_robotLeftHand->getFingerAxisVelocityFeedback(
        m_icubLeftFingerAxisVelocityFeedback);
  ...
  if (m_useRightHand) {
    ...
    m_robotRightHand->getFingerAxisValueReference(
        m_icubRightFingerAxisValueReference);
    m_robotRightHand->getFingerAxisVelocityFeedback(
        m_icubRightFingerAxisVelocityFeedback);
  ...
  if (m_state == HapticGloveFSM::Running) {
    ...
    if (false) {
      if (m_timeNow - m_timePreparation < 10.0) {
        ...
        m_gloveRightHand->setFingersForceReference(
    ...
    */
    if (m_useLeftHand) {
      ...
      if (!m_retargetingLeftHand->retargetHumanMotionToRobot(
              humanJointAngles)) {
      ...
      m_retargetingLeftHand->getRobotJointReferences(
    ...
    if (m_useRightHand) {
      ...
      if (!m_retargetingRightHand->retargetHumanMotionToRobot(
              humanJointAngles)) {
        yError() << "[HapticGloveModule::updateModule()] "
      ...
      m_retargetingRightHand->getRobotJointReferences(
    ...
    if (m_useLeftHand) {
      ...
      m_robotLeftHand->getEstimatedMotorsState(
          axisFeedbackValuesEstimationKF, axisFeedbackVelocitiesEstimationKF,
          axisFeedbackAccelerationEstimationKF, feedbackAxisCovEstimationKF,
          axisReferenceValuesEstimationKF, axisReferenceVelocitiesEstimationKF,
          axisReferenceAccelerationEstimationKF, referenceAxisCovEstimationKF);

      for (int i = 0; i < m_icubLeftFingerAxisValueReference.size(); i++) {
        m_icubLeftFingerAxisValueError(i) = axisReferenceValuesEstimationKF[i] -
                                            axisFeedbackValuesEstimationKF[i];
        m_icubLeftFingerAxisVelocityError(i) =
            axisReferenceVelocitiesEstimationKF[i] -
            axisFeedbackVelocitiesEstimationKF[i];

        m_icubLeftFingerAxisValueErrorSmoothed(i) =
            m_icubLeftFingerAxisValueError(i);
        m_icubLeftFingerAxisVelocityErrorSmoothed(i) =
            m_icubLeftFingerAxisVelocityError(i);
      }
      ...
      yInfo() << "axisReferenceValuesEstimationKF: "
              << axisReferenceValuesEstimationKF;
      yInfo() << "axisFeedbackValuesEstimationKF: "
              << axisFeedbackValuesEstimationKF;
      yInfo() << "axisReferenceVelocitiesEstimationKF: "
              << axisReferenceVelocitiesEstimationKF;
      yInfo() << "axisFeedbackVelocitiesEstimationKF: "
              << axisFeedbackVelocitiesEstimationKF;
      ...
      */
      ...
      m_retargetingLeftHand->retargetHapticFeedbackFromRobotToHuman(
          m_icubLeftFingerAxisValueErrorSmoothed,
          m_icubLeftFingerAxisVelocityErrorSmoothed);
      m_retargetingLeftHand->getForceFeedbackToHuman(
          m_gloveLeftForceFeedbackReference);
      m_retargetingLeftHand->getVibroTactileFeedbackToHuman(
    ...
    if (m_useRightHand) {
      ...
      m_robotRightHand->getEstimatedMotorsState(
          axisFeedbackValuesEstimationKF, axisFeedbackVelocitiesEstimationKF,
          axisFeedbackAccelerationEstimationKF, feedbackAxisCovEstimationKF,
          axisReferenceValuesEstimationKF, axisReferenceVelocitiesEstimationKF,
          axisReferenceAccelerationEstimationKF, referenceAxisCovEstimationKF);
      ...
      for (int i = 0; i < m_icubRightFingerAxisValueReference.size(); i++) {
        m_icubRightFingerAxisValueError(i) =
            axisReferenceValuesEstimationKF[i] -
            axisFeedbackValuesEstimationKF[i];
        m_icubRightFingerAxisVelocityError(i) =
            axisReferenceVelocitiesEstimationKF[i] -
            axisFeedbackVelocitiesEstimationKF[i];

        m_icubRightFingerAxisValueErrorSmoothed(i) =
            m_icubRightFingerAxisValueError(i);
      ...
      m_rightAxisValueErrorSmoother->computeNextValues(
          m_icubRightFingerAxisValueError);
      m_icubRightFingerAxisValueErrorSmoothed =
          m_rightAxisValueErrorSmoother->getPos();

      m_rightAxisVelocityErrorSmoother->computeNextValues(
          m_icubRightFingerAxisVelocityError);
      ...
      */
      ...
      m_retargetingRightHand->retargetHapticFeedbackFromRobotToHuman(
          m_icubRightFingerAxisValueErrorSmoothed,
          m_icubRightFingerAxisVelocityErrorSmoothed);
      m_retargetingRightHand->getForceFeedbackToHuman(
          m_gloveRightForceFeedbackReference);
      m_retargetingRightHand->getVibroTactileFeedbackToHuman(
          m_gloveRightBuzzMotorReference);
    }
    ...
    if (m_useLeftHand) {
      ...
      m_robotLeftHand->setFingersJointReference(
          m_icubLeftFingerJointsReference);
      ...
      m_gloveLeftHand->setFingersForceReference(
          m_gloveLeftForceFeedbackReference);
    ...
    if (m_useRightHand) {
      ...
      yInfo() << "m_icubRightFingerJointsReference\n"
              << m_icubRightFingerJointsReference.toString();
      m_robotRightHand->setFingersJointReference(
          m_icubRightFingerJointsReference);
    ...
    int axisNumber =
        int(dTime / CouplingConstant); // both operands needs to be integer
    ...
    if (m_useLeftHand) {
      if (axisNumber >=
        ...
        if (m_getHumanMotionRange) {
          ...
          m_gloveLeftHand->getHumanMotionRange(humanHandJointRangeMin,
        ...
        m_robotLeftHand->LogDataToCalibrateRobotMotorsJointsCouplingSin(
            time, axisNumber);
        if (m_getHumanMotionRange) {
    ...
    if (m_useRightHand) {
      if (axisNumber >=
        ...
        if (!m_robotRightHand->isRobotPrepared()) {
          ...
          if (m_getHumanMotionRange) {
            std::vector<double> humanHandJointRangeMin, humanHandJointRangeMax;
            m_gloveRightHand->getHumanMotionRange(humanHandJointRangeMin,
                                                  humanHandJointRangeMax);
        ...
        //                * 2.0);
        m_robotRightHand->LogDataToCalibrateRobotMotorsJointsCouplingSin(
            time, axisNumber);
        if (m_getHumanMotionRange) {
    ...
    if (m_useLeftHand && m_useRightHand) {
      if (m_robotLeftHand->isRobotPrepared() &&
          m_robotRightHand->isRobotPrepared()) {
        m_state = HapticGloveFSM::Running;
        m_timePreparation = yarp::os::Time::now();
      }
...
void HapticGloveModule::logData() {
  ...
  if (m_enableLogger) {
    ...
    if (m_useLeftHand) {
      ...
      /* Robot */
      // Axis
      std::vector<double> icubLeftFingerAxisFeedback,
      ...
      if (m_robot == "icub") {
        ...
        m_robotLeftHand->getMotorCurrentFeedback(
            icubLeftHandMotorCurrentFeedback);
        m_robotLeftHand->getMotorCurrentReference(
            icubLeftHandMotorCurrentReference);
        ...
        // pwm
        std::vector<double> icubLeftHandMotorPWMFeedback,
      ...
      for (int i = 0;
           i < m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis();
           i++) {
        icubLeftFingerAxisError[i] = m_icubLeftFingerAxisValueError(i);
        icubLeftFingerAxisErrorSmoothed[i] =
            m_icubLeftFingerAxisValueErrorSmoothed(i);
      }
      ...
      m_logger->add(m_logger_prefix + "_icubLeftHandMotorAxisValuesFeedbackKF",
                    axisFeedbackValuesEstimationKF);
      m_logger->add(m_logger_prefix +
                        "_icubLeftHandMotorAxisVelocitiesFeedbackKF",
                    axisFeedbackVelocitiesEstimationKF);
      m_logger->add(m_logger_prefix +
                        "_icubLeftHandMotorAxisAccelerationFeedbackKF",
                    axisFeedbackAccelerationEstimationKF);
      ...
      m_logger->add(m_logger_prefix + "_icubLeftHandMotorAxisValuesReferenceKF",
                    axisReferenceValuesEstimationKF);
      m_logger->add(m_logger_prefix +
                        "_icubLeftHandMotorAxisVelocitiesReferenceKF",
                    axisReferenceVelocitiesEstimationKF);
      m_logger->add(m_logger_prefix +
                        "_icubLeftHandMotorAxisAccelerationReferenceKF",
                    axisReferenceAccelerationEstimationKF);
      m_logger->add(m_logger_prefix + "_icubLeftHandMotorAxisCovReferenceKF",
                    referenceAxisCovEstimationKF);

      // Joints
      std::vector<double> icubLeftFingerJointsReference,
      ...
      std::vector<double> gloveLeftFingerForceFeedback(
          m_gloveLeftHand->getNoOfForceFeedback());
    ...
    if (m_useRightHand) {
      ...
      if (m_robot == "icub") {
        ...
        m_robotRightHand->getMotorCurrentFeedback(
            icubRightHandMotorCurrentFeedback);
        m_robotRightHand->getMotorCurrentReference(
            icubRightHandMotorCurrentReference);
        ...
        std::vector<double> icubRightHandMotorPWMFeedback,
      ...
      for (int i = 0;
           i < m_robotRightHand->controlHelper()->getNumberOfActuatedAxis();
           i++) {
        icubRightFingerAxisError[i] = m_icubRightFingerAxisValueError(i);
        icubRightFingerAxisErrorSmoothed[i] =
            m_icubRightFingerAxisValueErrorSmoothed(i);
      }
      ...
      m_logger->add(m_logger_prefix + "_icubRightHandMotorAxisValuesFeedbackKF",
                    axisFeedbackValuesEstimationKF);
      m_logger->add(m_logger_prefix +
                        "_icubRightHandMotorAxisVelocitiesFeedbackKF",
                    axisFeedbackVelocitiesEstimationKF);
      m_logger->add(m_logger_prefix +
                        "_icubRightHandMotorAxisAccelerationFeedbackKF",
                    axisFeedbackAccelerationEstimationKF);
      m_logger->add(m_logger_prefix + "_icubRightHandMotorAxisCovFeedbackKF",
                    feedbackAxisCovEstimationKF);

      m_logger->add(m_logger_prefix +
                        "_icubRightHandMotorAxisValuesReferenceKF",
                    axisReferenceValuesEstimationKF);
      m_logger->add(m_logger_prefix +
                        "_icubRightHandMotorAxisVelocitiesReferenceKF",
                    axisReferenceVelocitiesEstimationKF);
      m_logger->add(m_logger_prefix +
                        "_icubRightHandMotorAxisAccelerationReferenceKF",
                    axisReferenceAccelerationEstimationKF);
      m_logger->add(m_logger_prefix + "_icubRightHandMotorAxisCovReferenceKF",
                    referenceAxisCovEstimationKF);

      // Joints
      std::vector<double> icubRightFingerJointsReference,
      ...
      std::vector<double> gloveRightFingerForceFeedback(
          m_gloveRightHand->getNoOfForceFeedback());
      for (int i = 0; i < m_gloveRightHand->getNoOfForceFeedback(); i++)
        gloveRightFingerForceFeedback[i] =
            m_gloveRightForceFeedbackReference(i);
...
bool HapticGloveModule::openLogger() {
  ...
  if (m_useLeftHand) {
    // Robot hand axis
    m_logger->create(
        m_logger_prefix + "_icubLeftFingerAxisReference",
        m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis());
    m_logger->create(
        m_logger_prefix + "_icubLeftFingerAxisFeedback",
        m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis());
    if (m_robot == "icub") {
      m_logger->create(
          m_logger_prefix + "_icubLeftHandMotorCurrnetFeedback",
          m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis());
      m_logger->create(
          m_logger_prefix + "_icubLeftHandMotorCurrnetReference",
          m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis());
      m_logger->create(
          m_logger_prefix + "_icubLeftHandMotorPWMFeedback",
          m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis());
      m_logger->create(
          m_logger_prefix + "_icubLeftHandMotorPWMReference",
          m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis());
    }
    m_logger->create(
        m_logger_prefix + "_icubLeftHandMotorPidOutputs",
        m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis());

    m_logger->create(
        m_logger_prefix + "_icubLeftFingerAxisVelocityFeedback",
        m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis());

    m_logger->create(
        m_logger_prefix + "_icubLeftHandMotorAxisValuesFeedbackKF",
        m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis());
    m_logger->create(
        m_logger_prefix + "_icubLeftHandMotorAxisVelocitiesFeedbackKF",
        m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis());
    m_logger->create(
        m_logger_prefix + "_icubLeftHandMotorAxisAccelerationFeedbackKF",
        m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis());
    m_logger->create(
        m_logger_prefix + "_icubLeftHandMotorAxisCovFeedbackKF",
        m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis(),
        9); // states: value, velocity, acceleration --> cov matrix size: 3X3=9

    m_logger->create(
        m_logger_prefix + "_icubLeftHandMotorAxisValuesReferenceKF",
        m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis());
    m_logger->create(
        m_logger_prefix + "_icubLeftHandMotorAxisVelocitiesReferenceKF",
        m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis());
    m_logger->create(
        m_logger_prefix + "_icubLeftHandMotorAxisAccelerationReferenceKF",
        m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis());
    m_logger->create(
        m_logger_prefix + "_icubLeftHandMotorAxisCovReferenceKF",
        m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis(), 9);

    // robot axis errors
    m_logger->create(
        m_logger_prefix + "_icubLeftFingerAxisError",
        m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis());
    m_logger->create(
        m_logger_prefix + "_icubLeftFingerAxisErrorSmoothed",
        m_robotLeftHand->controlHelper()->getNumberOfActuatedAxis());

    // Robot hand joints
    m_logger->create(
        m_logger_prefix + "_icubLeftFingerJointsReference",
        m_robotLeftHand->controlHelper()->getNumberOfActuatedJoints());
    m_logger->create(
        m_logger_prefix + "_icubLeftFingerJointsFeedback",
        m_robotLeftHand->controlHelper()->getNumberOfActuatedJoints());
    m_logger->create(
        m_logger_prefix + "_icubLeftFingerJointsExpectedKF",
        m_robotLeftHand->controlHelper()->getNumberOfActuatedJoints());
    m_logger->create(
        m_logger_prefix + "_icubLeftFingerJointsFeedbackKF",
        m_robotLeftHand->controlHelper()->getNumberOfActuatedJoints());

    // Human info comming from Glove
    m_logger->create(m_logger_prefix + "_humanLeftHandPose",
                     m_gloveLeftHand->getNoHandLinks(), 7);

    m_logger->create(m_logger_prefix + "_humanLeftGlovePose",
                     m_gloveLeftHand->getNoGloveLinks(), 7);
    m_logger->create(m_logger_prefix + "_gloveLeftSensors",
                     m_gloveLeftHand->getNoSensors());
    m_logger->create(m_logger_prefix + "_gloveLeftForceFeedback",
                     m_gloveLeftHand->getNoOfForceFeedback());
  ...
  if (m_useRightHand) {
    // Robot hand axis
    m_logger->create(
        m_logger_prefix + "_icubRightFingerAxisReference",
        m_robotRightHand->controlHelper()->getNumberOfActuatedAxis());
    m_logger->create(
        m_logger_prefix + "_icubRightFingerAxisFeedback",
        m_robotRightHand->controlHelper()->getNumberOfActuatedAxis());
    if (m_robot == "icub") {
      m_logger->create(
          m_logger_prefix + "_icubRightHandMotorCurrnetFeedback",
          m_robotRightHand->controlHelper()->getNumberOfActuatedAxis());
      m_logger->create(
          m_logger_prefix + "_icubRightHandMotorCurrnetReference",
          m_robotRightHand->controlHelper()->getNumberOfActuatedAxis());
      m_logger->create(
          m_logger_prefix + "_icubRightHandMotorPWMFeedback",
          m_robotRightHand->controlHelper()->getNumberOfActuatedAxis());
      m_logger->create(
          m_logger_prefix + "_icubRightHandMotorPWMReference",
          m_robotRightHand->controlHelper()->getNumberOfActuatedAxis());
    }
    m_logger->create(
        m_logger_prefix + "_icubRightHandMotorPidOutputs",
        m_robotRightHand->controlHelper()->getNumberOfActuatedAxis());

    m_logger->create(
        m_logger_prefix + "_icubRightFingerAxisVelocityFeedback",
        m_robotRightHand->controlHelper()->getNumberOfActuatedAxis());

    m_logger->create(
        m_logger_prefix + "_icubRightHandMotorAxisValuesFeedbackKF",
        m_robotRightHand->controlHelper()->getNumberOfActuatedAxis());
    m_logger->create(
        m_logger_prefix + "_icubRightHandMotorAxisVelocitiesFeedbackKF",
        m_robotRightHand->controlHelper()->getNumberOfActuatedAxis());
    m_logger->create(
        m_logger_prefix + "_icubRightHandMotorAxisAccelerationFeedbackKF",
        m_robotRightHand->controlHelper()->getNumberOfActuatedAxis());
    m_logger->create(
        m_logger_prefix + "_icubRightHandMotorAxisCovFeedbackKF",
        m_robotRightHand->controlHelper()->getNumberOfActuatedAxis(), 9);

    m_logger->create(
        m_logger_prefix + "_icubRightHandMotorAxisValuesReferenceKF",
        m_robotRightHand->controlHelper()->getNumberOfActuatedAxis());
    m_logger->create(
        m_logger_prefix + "_icubRightHandMotorAxisVelocitiesReferenceKF",
        m_robotRightHand->controlHelper()->getNumberOfActuatedAxis());
    m_logger->create(
        m_logger_prefix + "_icubRightHandMotorAxisAccelerationReferenceKF",
        m_robotRightHand->controlHelper()->getNumberOfActuatedAxis());
    m_logger->create(
        m_logger_prefix + "_icubRightHandMotorAxisCovReferenceKF",
        m_robotRightHand->controlHelper()->getNumberOfActuatedAxis(), 9);

    // robot axis errors
    m_logger->create(
        m_logger_prefix + "_icubRightFingerAxisError",
        m_robotRightHand->controlHelper()->getNumberOfActuatedAxis());
    m_logger->create(
        m_logger_prefix + "_icubRightFingerAxisErrorSmoothed",
        m_robotRightHand->controlHelper()->getNumberOfActuatedAxis());

    // Robot hand joints
    m_logger->create(
        m_logger_prefix + "_icubRightFingerJointsReference",
        m_robotRightHand->controlHelper()->getNumberOfActuatedJoints());
    m_logger->create(
        m_logger_prefix + "_icubRightFingerJointsFeedback",
        m_robotRightHand->controlHelper()->getNumberOfActuatedJoints());
    m_logger->create(
        m_logger_prefix + "_icubRightFingerJointsExpectedKF",
        m_robotRightHand->controlHelper()->getNumberOfActuatedJoints());
    m_logger->create(
        m_logger_prefix + "_icubRightFingerJointsFeedbackKF",
        m_robotRightHand->controlHelper()->getNumberOfActuatedJoints());

    // Human info comming from Glove
    m_logger->create(m_logger_prefix + "_humanRightHandPose",
                     m_gloveRightHand->getNoHandLinks(), 7);

    m_logger->create(m_logger_prefix + "_humanRightGlovePose",
                     m_gloveRightHand->getNoGloveLinks(), 7);
    m_logger->create(m_logger_prefix + "_gloveRightSensors",
                     m_gloveRightHand->getNoSensors());
    m_logger->create(m_logger_prefix + "_gloveRightForceFeedback",
                     m_gloveRightHand->getNoOfForceFeedback());
...
#else
  yInfo()
      << "[HapticGloveModule::openLogger] option is not active in CMakeLists.";
