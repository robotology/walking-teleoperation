...
class Retargeting
{
    ...
    std::vector<double> m_fingerForceFeedback; // values
    std::vector<double> m_fingerBuzzFeedback; // values

    std::vector<FingerAxisRelation> m_fingerAxisRelation;

public:
    Retargeting(const size_t noAllAxis,
    ...
    bool retargetForceFeedbackFromRobotToHuman(const yarp::sig::Vector& axisValueError,
                                               const yarp::sig::Vector& axisVelocityError);

    bool retargetVibroTactileFeedbackFromRobotToHuman();

    bool retargetHapticFeedbackFromRobotToHuman(const yarp::sig::Vector& axisValueError,
                                                const yarp::sig::Vector& axisVelocityError);
